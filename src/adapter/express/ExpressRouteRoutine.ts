...
export async function ExpressRouteRoutine(
  routine: IProxiedApiRoute,
  request: IApiRouteRequest
) {

  let requestProxies: IApiRequestProxy[] = routine.requestProxies;
  ...
  // Execute Function
  try {
    let routineResponse = await routine.resolver(request);
